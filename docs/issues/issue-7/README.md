...
| 2026-08-10 | 三項待確認查證完畢：成本非問題（實測）、屬疏漏而非取捨（issue-1 技術分析原文）、migration 不可行（循環 import）→ 方案收斂為「呼叫端補索引 + verify 覆蓋率檢查」，狀態轉待實作 |
| 2026-08-10 | 實作完成：兩個呼叫點補 `indexPromotedSession`；覆蓋率改放 `stats`（R1）；`test-repo-promotion.sh` 加索引覆蓋與 tree 召回斷言 |
