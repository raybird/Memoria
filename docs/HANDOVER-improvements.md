...
**P1–P10 全數完成。** 記憶智能主線 **UFL Phase 0 + 1 + 2 + 3 全數 ship**（見 `docs/RFC-utility-feedback.md`）：Phase 1 = `recall_id` + Migration 6 + `recordRecallOutcome` + endpoint + SDK + adapter 回報；Phase 2 = `buildCalibration` confidence×utility 分桶校準；Phase 3(b) = Migration 7 `memory_utility` per-memory 歸因 + `applyUtilityWeighting` utility-weighted 召回排序 + prune retention；**Phase 3(a) = Migration 8 explicit 累加器 + `effectiveUtility`（explicit 高保真、與 reuse 分開累計永不相加、存在即凌駕）+ SDK `markRecallUseful`**。全數零觀測即 byte-identical、可回退。**語意召回 MVP 亦已 ship**（`docs/RFC-semantic-recall.md` `phase-1-shipped`：`mode:'vector'` = 本地 e5 embedding + libSQL 原生向量 + RRF，選用、fail-open）——UFL 標尺就位，**下一步 = 讓真實資料累積，用 route_mode 分組 utility uplift 客觀評測語意 vs 字面**。P5 的 CJK 範圍對齊方向仍為待決。
