...
```bash
bash install.sh --version 1.22.0 --install-dir "$HOME/.local/share/memoria"
```

可先檢查將要下載的 URL，不會寫入任何檔案：

```bash
bash install.sh --version 1.22.0 --print-release-url
```
...
```bash
bash install.sh \
  --artifact ./memoria-linux-x64-v1.22.0.tar.gz \
