...
- **v1.26.0 = 語意召回交付鏈補完(issue-10 + issue-12)**(2026-08-13):(a) **release tarball 終於帶上 `skills/memoria-vector`**——`install.sh` 裝出來的 Memoria 先前**不可能**跑語意召回,而且是靜默的(helper 找不到 → `unavailable` → fail-open 退回字面召回,無錯誤無警告)。v1.23.1 用 `files` 修好了 npm 那半邊,這半邊被留下,同一個症狀換一個成因活了下來。逐檔複製而非 `cp -R`(helper 的 `node_modules` 是 ~850MB devDeps,必須維持 opt-in),打包腳本**雙向斷言**:該有的三個檔要在、`node_modules` 不准在。(b) **`doctor` 開始報向量層**——`inspectVectorLayer()` 放在 `core/recall-vector.ts`(純加法)而非 doctor 命令裡,因為重寫一份解析順序會漂移,然後自信地診斷一個 recall 從不載入的 helper。**未設 `LIBSQL_URL` 算通過**(opt-in 沒開 ≠ 不健康),`DoctorCheck` 型別與 `--json` 契約都沒動。實作中補掉一個規格漏寫的坑:`resolveHelperScript()` 對 override 原樣回傳不驗存在性,照抄會把打錯字的 override 報成健康。
- **⚠ 2026-08-13 由下游回報開出兩個新 issue,其中一個影響本機日常使用**:[issue-15](issues/issue-15/README.md) 自然語言中文查詢召不回(整句成單一 token,要求逐字出現;實測使用者真實 `~/.memoria` 確認)、[issue-14](issues/issue-14/README.md) `db_integrity` 誤報且訊息不帶證據(根因未定位,本 issue 只修可診斷性)。**issue-15 優先於 issue-14**:前者影響使用者本人每天的召回品質,後者只影響一個下游的健康訊號可讀性。
- **下一步**:(a) **讓真實 recall/outcome 資料累積**——四種 route 都可用了,現在純粹缺使用量,且 issue-9 修好後 calibration 讀數才有意義;(b) **語意召回交付鏈已補完**——issue-10/12 於 v1.26.0、issue-11 於 v1.26.1 交付,三段(沒交付 / 交付了跑不動 / 壞了沒人說)全數收斂;剩下的 [issue-13](issues/issue-13/README.md) 是能力缺口而非交付缺口,且有下游在等第一階段;(c) **[issue-8](issues/issue-8/README.md) 待評估**:vector helper 每次 spawn 吃 624MB 且無並行上限(潛在風險,非已發生事故;優先序理由見上);(d) 工程債:`repo-facade` 抽取(等下次動 repo 邏輯)。
...
| v1.29.0 | 記憶的嚴重度與範圍 | `brief` 新增 pinned 區（`retention='durable'` 免輪替）、依 cwd 產出 per-project BRIEF（`--global` 可還原）；新增 `mark` 讓既有記憶（含 git 促升的決策）可被標記；`doctor` 報向量索引覆蓋率
...
| 代號 | 項目 | 狀態 | 備註 |
|------|------|------|------|
| — | **向量層覆蓋率讀數** | `planned`(2026-08-25) | 已展開為 `docs/issues/issue-18/`(驗收條件已核准,未實作)。覆蓋不足是靜默的,且已發生兩次:issue-7 前 payload 縮水到 10 個 session 只涵蓋 3 個;2026-08-13 一筆新記憶的 node 建了但沒 ingest,語意召回查不到它 11 天,零訊號。收在 `doctor`(issue-12 已有 `inspectVectorLayer()`)而非 `stats`(純 SQLite,加 libSQL 讀取會把 Small 變 Medium)。**注意**:量 `memoria_vectors` 一律要加 `NOT INDEXED`——不加會回 0,那是 stock sqlite3 誤用 libSQL 向量索引的假象,曾據此誤判成「向量庫是空的」 |
| — | **記憶改為可執行約束的形狀** | `experiment`(2026-08-25) | 目前 event content 只有 `decision`/`rationale`/`impact_level`,是給人讀的會議紀錄格式;agent 需要的是「管哪個範圍、違反的症狀、怎麼自檢」。**刻意不先加欄位**:issue-17 的根因正是 `impact_level` 沒有任何機制強迫填寫,於是兩條寫入路徑各自硬寫常數;直接加 `failure_mode`/`symptom`/`self_check` 會在一年後得到三個新的 `impact_level`。改為先用既有的 `memory_attributes.note` 做實驗——2026-08-25 已替 5 則 pinned 記憶補上「範圍/症狀/自檢」,issue-18 的 AC-4 讓它在 brief 顯示得到。**重估判準**:那段文字有沒有真的改變過 agent 的行為?有才值得升格成欄位並設計填寫的強制點 |
| — | **feedback 迴圈的結構性衰減** | `deferred`(2026-08-25,**有重估判準**) | 現況 60 筆 recall telemetry 只有 4 筆 explicit outcome,其中 1 筆還是調查時手動補的;校準曲線建不起來(`scoped=1`)。診斷正確——CLAUDE.md 要求「用到了就回報」,但那個動作被安排在 agent 已經拿到價值之後,是最不可能執行的時刻。**但延後,因為它的價值剛被 issue-17 削弱**:「重要記憶不被輪替」已改由 `retention='durable'` 的明示標記承擔(pinned 區),比累積效用便宜且可解釋,UFL 的排序角色因此縮水成 tiebreaker。另外**負向路徑其實沒有衰減**——2026-08-24 發現一筆記憶過期時,回報是透過 `remember --supersedes` 完成的,糾正的動機天然存在。**重估判準**:pinned 運作一段時間後,若它已解決「重要記憶留在 context」,UFL 只需要活著、不需要被餵飽;要投資前先讓 `routeUtility` 累積到足以回答「語意是否勝過字面」 |
...
| — | **release tarball 不含 vector helper** | **`done`**(v1.26.0) | 已展開為 [issue-10](issues/issue-10/README.md)。`package-release-artifacts.sh:47` 只複製 `memoria-memory-sync`——**v1.23.1 修的是 npm 那半邊,tarball 這半邊維持原狀**,症狀完全相同:`resolveHelperScript()` 回 null → `status:'unavailable'` → fail-open 退回字面召回,無錯誤無警告。`test-no-clone-install.sh` 完全沒提到 vector,補斷言是防復發的必要條件 |
| — | **helper 的 transformers 分類錯邊** | **`done`**(2026-08-13,**採方案 D**) | 分類**不動**——移進 `dependencies` 與 helper 自述的存在理由(「Deliberately OUTSIDE Memoria's core dependencies」)相衝突,`optionalDependencies` 只是把陷阱從 `--omit=dev` 換名成 `--omit=optional`。**解法是可見性而非搬家**:錯誤訊息點名 `--omit=dev` / `NODE_ENV=production` 是最常見成因(舊訊息叫人「run npm install」,對剛用 `--omit=dev` 跑完而且成功的人來說像自相矛盾),README 加 callout,`doctor` 那半邊 v1.26.0 已交付。原始 backlog 敘述如下 | 已展開為 [issue-11](issues/issue-11/README.md)。`@huggingface/transformers` 在 devDependencies,但 `embed.mjs` 的預設 provider 就是 `local` 且執行期動態 import 它 → `npm install --omit=dev` / `NODE_ENV=production` 裝出跑不動的 helper。**有明確錯誤訊息,不是靜默失效**,故低於 issue-10 一級。四個候選方案(移 deps / optionalDeps / 只改文件 / 只改文件+doctor 檢查)各有代價——~850MB 不是每個使用者都該付,需拍板 |
| — | **doctor 不檢查向量層** | **`done`**(v1.26.0) | 已展開為 [issue-12](issues/issue-12/README.md)。`doctor.ts:28-35` 六項全是路徑存在性,issue-10/11 兩種失效沒有任何指令會說出來。**實作時的界線**:opt-in 未啟用 ≠ 不健康——`ok` 是 `checks.every()`,把「沒設 `LIBSQL_URL`」算成失敗會讓所有沒用語意召回的人拿到紅燈,那比不檢查更糟(同 issue-7 R1 對 `verify` 的判斷) |
| — | **HTTP 缺 `/v1/brief`** | **第一階段 `done`**(v1.27.0);**第二階段 `--server` `planned`,理由已改** | 第一階段發版後實接推翻了「端點齊全就夠」——**agent 打的是 CLI 指令不是 HTTP**(使用者 `CLAUDE.md` 直接寫 `memoria recall "<查詢>"`),所以 sidecar 仍需一個 CLI→HTTP 的 shim,而那個 shim 得重寫 `recall` 的輸出渲染(`recall.ts:55-77` 的標頭／版面／`recall_id:` 尾行)。那正是第一階段為 `brief` 刻意避開的「repo 外的第二實作」,只是換到 agent 讀最頻繁的 `recall` 身上。⚠ **需求是「渲染所有權留在 Memoria」,`--server` 只是形狀**——任何能讓 CLI 維持唯一渲染者的做法都成立(含比旗標更小的,例如讓 `recall` 認得一個 endpoint 環境變數)。下游最初附帶的兩個理由都已被實測推翻:(a) 免 ABI 配對——`cli.ts:5` → `core/index.js` → `db/connection.ts:1` 是頂層 import,加旗標改不了,要延遲載入才行;(b) 縮小 image——better-sqlite3 只有 **12M**,而 `skills/memoria-vector/node_modules` 是 **852M**(`onnxruntime-node` 513M / `@huggingface/transformers` 146M / `onnxruntime-web` 130M,本機與下游量測一致)。**雙方原本都猜大頭是 transformers,錯的。** image 的縮減來自語意召回搬到 sidecar 側,與 `--server` 怎麼做無關 | 已展開為 [issue-13](issues/issue-13/README.md)。sidecar 部署不成立的兩個缺口:無 brief 端點 + 主命令一律直開本地 SQLite(`src/` 內查無 `--server`)。**feedback 不缺**(`server.ts:343` 的 `/v1/recall/:id/outcome`,只是命名與 CLI 不一致)。建議拆兩段:先做 `/v1/brief`(`queryBrief`/`renderBrief` 已是純函式,包 handler 即可,但要決定回傳 JSON 或 markdown、以及**不該寫檔**),`--server` 另議——那動到每個命令的執行模型與 `latency_ms` 的意義 |
| — | **中文片語拿到連續子字串語意** | **`done`**(2026-08-17,未發版) | 已展開為 [issue-16](issues/issue-16/README.md),是 issue-15 的直接延續。`記憶`→5 筆、`召回`→3 筆,但 `記憶召回`→**0 筆**,而 `memory recall`→3 筆。成因兩層疊加:(a) 4 字查詢產出的 3-gram 全是跨詞邊界的垃圾(`記憶召`/`憶召回`),承載語意的 2 字詞在 trigram 索引上結構性不可達;(b) `queryRecallLike`(`recall.ts:523`)比對的是**整個原始查詢字串**而非 token,所以救援網本身也是逐字的。修法是 LIKE fallback 改對 token 做 OR。⚠ **動工前要量精確度**——LIKE 路徑沒有 bm25(`ORDER BY timestamp DESC`),OR 放寬可能撈回「時間近但不相關」的列;issue-15 的教訓正是這類要量不要推 |
| — | **自然語言中文查詢召不回** | **`done`**(2026-08-14) | 已展開為 [issue-15](issues/issue-15/README.md)。`TOKEN_SPLIT_PATTERN` 保留全部 CJK 字元且中文無空白,**一整句問句 tokenize 後是一個 token**,然後被要求整句逐字出現。實測使用者真實 `~/.memoria`:`精準停`/`一律用` 各 2 筆(連續子字串,trigram FTS 本來就行),但 `停止伺服器行程要注意什麼` **0 筆**;`語意召回的 helper 裝在哪裡` 的 3 筆是 `helper` 帶來的,不是中文。**不是索引能力問題**——`recall_fts` 用 trigram,有能力,是我們餵整句給它。issue-9 記過同一個 tokenizer 行為但把後果侷限在 confidence,實際上也吃掉召回。⚠ **影響本 repo 使用者的日常路徑**(`CLAUDE.md` 要 agent 打自然語言中文查詢),目前只有 opt-in 的 vector 模式救得回來。動工前有 5 項待決(切分策略/精確度代價/與 issue-9 的 confidence 耦合/只改查詢端或連索引/既有純函式測試預期) |
| — | **`db_integrity` 誤報且不可診斷** | **`done`**(v1.27.1) | 已展開為 [issue-14](issues/issue-14/README.md)。下游正式環境 `/v1/health` 穩定 `ok:false`,16 項只有 `db_integrity` 失敗,但同檔案新連線 `quick_check` 回 `ok`(單列、非 undefined、非錯誤字串)。**根因未定位**:本機依 server 形狀(長生命週期 readonly + 另一條 rw,`journal_mode=delete`)跑 idle/持鎖/commit 後/DDL 後/25k 列成長五種情境,**全部 ok**,pooled 連線假設單獨不成立;未排除的是多行程。⚠ 本 issue 修的是**可診斷性不是誤報**——`verify.ts:82-84` 的失敗訊息是常數字串,把實際值丟掉,且 `.get()` 回 `undefined` 時與真正損壞不可分辨 |
...
- **Release SOP 是兩個命令,其餘由腳本執行**:`pnpm run release:prepare <level>` → 手動編 CHANGELOG → `pnpm run release:publish`。細節在 `scripts/release.sh`,**不要回去手打 git 指令**。`publish` 的守門每一條都對應一次真實踩雷:只有 bump 檔案可以是髒的(否則 release commit 會吞掉未提交的功能)、`build` 先於 `docs-check`、annotated tag 且明示推 ref(`--follow-tags` 只推 annotated,lightweight 會被**靜默略過**——push 成功、main 更新、workflow 從未觸發、輸出毫無跡象)、workflow 必須在 2 分鐘內出現、最後對**下載回來的**產物再跑一次 parity。2026-08-13 發 v1.25.1 踩過那個 tag 雷,**成因是 `bump-version.mjs` 印出的 next steps 自己就是壞組合**,而 `RELEASE.md` 寫的是對的——照著剛跑完的腳本輸出做,比翻文件自然。現在 `bump-version.mjs` 不再印任何發版指令,沒有第二份可以跟腳本矛盾。
- **兩條交付路徑不得各自維護清單**(2026-08-13,issue-10 的結構性修正):npm(`package.json` 的 `files`)與 tarball(`package-release-artifacts.sh`)是兩份獨立清單,先前沒有交叉檢查,於是 v1.23.1 只動了其中一份,同一個缺陷在另一條路徑上活了三個版本。`scripts/check-delivery-parity.mjs` 現在**從 `npm pack --dry-run --json` 推導**(npm 自己的答案,不是重寫它的 glob 語意),每個 packed 路徑要嘛在 tarball、要嘛在 `DELIVERED_ELSEWHERE` 連同理由被列出,否則 build 失敗。**新增交付檔案時不要回頭去手動列白名單**——那正是被取代掉的東西。
- **helper 的執行契約有版控外的下游**(2026-08-13):一個下游容器化部署(代稱 **downstream-cli-container**,**細節不入版控文件**)把 `skills/memoria-vector` 打進自己的 image。⚠ 已知**兩個**下游容器部署,形態相反,代稱要分清:`downstream-cli-container`(agent 打 CLI、自帶 helper)與 `downstream-http-sidecar`(純 HTTP client、不需要 CLI 或 `MEMORIA_HOME`、未設 `LIBSQL_URL` 故一直是純字面召回)。本條只涉及前者。動 `resolveHelperScript()` 的解析順序,或 helper 的 stdin/stdout 契約,會波及它——那些是跨 repo 的隱性介面,本 repo 的測試涵蓋不到。改動時要在 CHANGELOG 明寫。

  ⚠ **helper 的檔案路徑本身也已經是事實上的契約**(2026-08-17 得知):該下游的 compose 把 `MEMORIA_VECTOR_RECALL_CMD` 寫成 npm 全域安裝下的**完整絕對路徑**(`…/node_modules/@raybird.chen/memoria/skills/memoria-vector/vector-recall.mjs`)。所以**搬動 helper 位置對他們是靜默失效**——CLI 照跑、`recall` 照回、只是永遠退回非語意路徑,沒有任何錯誤訊息。那是他們寫死路徑的選擇、他們自己的風險,但這是「動了會弄壞別人而且對方不會立刻知道」的少數幾件事之一,**要動就先知會**。
  ｜ 反過來,v1.28.1 的 helper 契約在他們那條路徑上是**實際驗過**而非「沒觸發」:14 項閘門全過,含 `route_mode=hybrid_vector`、唯讀掛載、非特權使用者、以及 `better-sqlite3` 的 ABI prebuild(Node 18.20.8 對 18.20.4,同 major)。
