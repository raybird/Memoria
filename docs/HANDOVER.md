...
| — | **中文片語拿到連續子字串語意** | `planned`(**待量測**,2026-08-17) | 已展開為 [issue-16](issues/issue-16/README.md),是 issue-15 的直接延續。`記憶`→5 筆、`召回`→3 筆,但 `記憶召回`→**0 筆**,而 `memory recall`→3 筆。成因兩層疊加:(a) 4 字查詢產出的 3-gram 全是跨詞邊界的垃圾(`記憶召`/`憶召回`),承載語意的 2 字詞在 trigram 索引上結構性不可達;(b) `queryRecallLike`(`recall.ts:523`)比對的是**整個原始查詢字串**而非 token,所以救援網本身也是逐字的。修法是 LIKE fallback 改對 token 做 OR。⚠ **動工前要量精確度**——LIKE 路徑沒有 bm25(`ORDER BY timestamp DESC`),OR 放寬可能撈回「時間近但不相關」的列;issue-15 的教訓正是這類要量不要推 |
