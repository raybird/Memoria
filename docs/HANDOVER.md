...
- **下一步**:(a) **讓真實 recall/outcome 資料累積**——四種 route 都可用了,現在純粹缺使用量,且 issue-9 修好後 calibration 讀數才有意義;(b) **issue-11 待拍板**——issue-10/12 已於 v1.26.0 交付,那條交付鏈只剩「交付了但缺 embedding 後端」這一段,而它是四選一的成本取捨,不是技術問題;(c) **[issue-8](issues/issue-8/README.md) 待評估**:vector helper 每次 spawn 吃 624MB 且無並行上限(潛在風險,非已發生事故;優先序理由見上);(d) 工程債:`repo-facade` 抽取(等下次動 repo 邏輯)。
