...
- **Release SOP**:`bump-version.mjs` → CHANGELOG 從 `[Unreleased]` 提升 → guards → tests → commit(`Release vX.Y.Z`)→ `git tag -a` → **`git push origin main` + `git push origin refs/tags/vX.Y.Z`** → `release.yml` 自動發 npm + GitHub Release。**明示推 tag ref,不用 `--follow-tags`**——那個旗標只推 annotated tag,lightweight tag 會被**靜默略過**:push 成功、main 更新、workflow 從未觸發,輸出裡沒有任何跡象。2026-08-13 發 v1.25.1 時踩過一次,**成因是 `bump-version.mjs` 印出的 next steps 自己就是壞組合**(`git tag`(無 `-a`)+ `--follow-tags`),而 `RELEASE.md` 寫的是對的——照著剛跑完的腳本輸出做,比翻文件自然。已同時修掉腳本輸出與 RELEASE.md,讓 tag 類型不再決定發版會不會發生。
