...
import { bufferPendingRecall, reportRecallOutcome } from './utility-shadow.js'
...
export abstract class BaseAdapter {
    ...
    async afterResponse(response: AdapterResponse): Promise<void> {
        try {
            // UFL: report reuse of last turn's injected memory (fail-open, no-op if nothing buffered).
            await reportRecallOutcome(this.client, response.conversationId, { user: response.userMessage ?? '', assistant: response.response })
    ...
    protected async recallForContext(context: AdapterContext): Promise<RecallContext> {
        ...
        // UFL: buffer injected hits (keyed by recall_id) so the next turn can report reuse. No-op without an id.
        bufferPendingRecall(context.conversationId, result.meta?.recall_id, hits)
