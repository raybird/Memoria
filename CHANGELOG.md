...
- **`brief` gained a pinned block** for `retention='durable'` memories, exempt from the `--days` window and `topK` (issue-17). The recent-decisions block is ordered by `timestamp` alone, and `impact_level` never entered it — nor could it have: `promoteSummary` hardcodes `'high'` and `rememberNote` hardcodes `'medium'`, so on the maintainer's corpus 31 of 39 decisions read `high` purely because of which code path wrote them. That field records provenance, not judgement, which is why severity now rides on an explicit marker instead. Pinned entries are one line each and leave the recency rotation entirely, so a memory whose value is "knowing it prevents an outage" no longer competes with routine ones for the same ten slots.
- **`brief` resolves the working directory to a registered repository** and writes `<knowledge>/BRIEF-<project>.md` scoped to it, plus an environment-class block that rides along into every project's file (issue-17). Longest-path matching with boundary awareness, scoped to this host. When nothing matches it falls back to the global file and **says so** rather than picking one of several repos. A memory whose `project` is not a registered repository name is environment-class; that classification is derived rather than declared — no new column, no migration — and the cost of deriving it is that registering such a project as a repo silently reclassifies its memories, so the count is reported on every run and the number moving is the signal. A per-project run also prints the exact absolute `@`-line to paste into that repo's `CLAUDE.md`, since the relative form used for the global brief does not resolve from there.

- **`doctor` reports vector index coverage** as `embedded / expected / missing` (issue-18). The failure this catches is not an empty store — it is a store that is SHORT, and it happened twice with no signal: before issue-7 the bridge payload narrowed to 3 of 10 sessions while ingest still answered `{"ok":true}`, and in 2026-08 one new memory's node existed but was never embedded, leaving semantic recall blind to it for eleven days. `missing > 0` fails the check and names the two-step ingest command; a store file that does not exist yet is a measured **zero**, not "unmeasurable", because someone who set `LIBSQL_URL` and never ingested is exactly the person whose semantic recall silently returns nothing. The denominator tracks the helper's `EMBEDDABLE_TYPES` (sessions + decision/skill events + memory nodes) and deliberately **includes** superseded memories, since ingest embeds them too — a denominator that can never be reached is a standing false alarm. Read directly from a `file:` libSQL URL rather than through the helper: that helper's stdin/stdout contract has consumers outside this repository (`docs/HANDOVER.md` §8), and widening it for a diagnostic would put a cross-repo interface at risk for a number; remote URLs report `not measured` instead of guessing. **An unset `LIBSQL_URL` still passes, and so does an overridden helper** — the same judgement issue-12 made, and it is not theoretical: adding this check turned issue-12's own "an overridden helper is not by itself an unhealthy install" assertion red, because with an override the index may not be filled by our ingest path at all, so a short count would not mean what it claims and the remedy printed alongside it would not apply.
- **`mark` now marks both halves of a CLI note** (issue-18). A note occupies two refs and `remember --durable` has always marked both; marking only the one that was named left the halves disagreeing, and recall's decay exemption is looked up per `hit.id`, so the unmarked half kept decaying silently. Refs with no counterpart (a git-promoted `gitdec-`) still get exactly one row — the pair is written only when it names an existing memory. The pairing rule is now one function instead of two copies.
- **The pinned block carries the marker's note** when one was recorded (issue-18), one indented line under the entry and nothing at all when there is none — the block earns its exemption from rotation by staying cheap.

### Changed
- **`brief` run inside a registered repository now writes `BRIEF-<project>.md` instead of `BRIEF.md`.** One global file cannot hold a cwd-filtered result: generation is manual and `CLAUDE.md` `@`-imports a fixed path, so whoever ran it last from whatever directory would decide what every project sees — unpredictable, which is worse than merely unfocused. **`--global` restores the previous single-file, all-projects behaviour in one flag.** An explicit `--project` is unaffected: only *detected* scope changes the filename, so existing callers keep writing `BRIEF.md`.
