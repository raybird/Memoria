...
db.prepare('DELETE FROM schema_migrations WHERE id IN (4,5)').run();
db.close();
console.log('  downgraded: dropped recall_fts + telemetry columns, removed migrations 4,5 (data kept)');
"
...
if (!migs.includes(4) || !migs.includes(5)) fail('migrations 4/5 not re-applied: ' + migs);
...
for (const c of ['query_hash','token_count','top_confidence']) if (!cols.has(c)) fail('telemetry missing column ' + c);
...
db.close();
if (n < 5) { console.error('  ✗ schema_migrations regressed: ' + n); process.exit(1); }
